...
interface HabilidadM {
    void usarHabilidad(PersonajeM enemigo);
    String getNombreHabilidad();
}

abstract class PersonajeM {
    ...
    public PersonajeM(String nombre, int vida, int ataque, int pociones) {
        this.nombre     = nombre;
        this.vida       = vida;
    ...
    public abstract void atacar(PersonajeM enemigo);
...
class GuerreroM extends PersonajeM implements HabilidadM {
    public GuerreroM(String nombre) { super(nombre, 120, 18, 3); }

    @Override
    public void atacar(PersonajeM enemigo) {
        ...
        if (enemigo.estaVivo()) System.out.println(enemigo.getNombre() + " le queda " + enemigo.getVida() + " de vida.");
        else System.out.println(enemigo.getNombre() + " ha caído ante tu fuerza!");
    }

    @Override
    public void usarHabilidad(PersonajeM enemigo) {
        int dano = getAtaque() * 2;
        enemigo.setVida(enemigo.getVida() - dano);
        System.out.println("⚔¡GOLPE BRUTAL! " + getNombre() + " aplasta a " + enemigo.getNombre() + " por " + dano + " puntos!");
        if (enemigo.estaVivo()) System.out.println(enemigo.getNombre() + " le queda " + enemigo.getVida() + " de vida.");
        else System.out.println(enemigo.getNombre() + " ha sido aplastado!");
    }
...
class MagoM extends PersonajeM implements HabilidadM {
    public MagoM(String nombre) { super(nombre, 80, 22, 2); }

    @Override
    public void atacar(PersonajeM enemigo) {
        int dano = getAtaque() + rand.nextInt(10);
        enemigo.setVida(enemigo.getVida() - dano);
        System.out.println(getNombre() + " lanza un rayo a " + enemigo.getNombre() + " por " + dano + " puntos de daño mágico!");
        if (enemigo.estaVivo()) System.out.println(enemigo.getNombre() + " le queda " + enemigo.getVida() + " de vida.");
        else System.out.println(enemigo.getNombre() + " ha sido fulminado!");
    }
    ...
    public void usarHabilidad(PersonajeM enemigo) {
        int dano = 50;
        enemigo.setVida(enemigo.getVida() - dano);
        System.out.println("¡BOLA DE FUEGO! " + getNombre() + " destruye a " + enemigo.getNombre() + " con " + dano + " puntos de daño!");
        if (enemigo.estaVivo()) System.out.println(enemigo.getNombre() + " le queda " + enemigo.getVida() + " de vida.");
        else System.out.println(enemigo.getNombre() + " ha sido incinerado!");
    }
...
class DragonOscuroM extends PersonajeM {
    public DragonOscuroM() { super("Dragón Oscuro", 150, 14, 0); }

    @Override
    public void atacar(PersonajeM enemigo) {
        int dano = getAtaque() + rand.nextInt(10);
        enemigo.setVida(enemigo.getVida() - dano);
        System.out.println("El " + getNombre() + " escupe fuego sobre " + enemigo.getNombre() + " por " + dano + " puntos!");
        if (enemigo.estaVivo()) System.out.println(enemigo.getNombre() + " le queda " + enemigo.getVida() + " de vida.");
        else System.out.println(enemigo.getNombre() + " ha caído ante el dragón!");
    }
}
...
class Main {
    public static void main(String[] args) {
        ...
        PersonajeM  heroe;
        HabilidadM  habilidad;

        if (claseElegida.equals("2")) {
            MagoM mago = new MagoM(nombre);
            heroe      = mago;
            habilidad  = mago;
        } else {
            GuerreroM guerrero = new GuerreroM(nombre);
            heroe              = guerrero;
            habilidad          = guerrero;
        }

        PersonajeM enemigo      = new DragonOscuroM();
